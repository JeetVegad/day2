...
with join1_cte(first_name,incdate,joindate) as (
SELECT firstname, incdate , joindate from employee5 A
INNER JOIN incentive B on A.empid = B.empid
) select  * from join1_cte

with join2_cte(first_name,incamount) as (
SELECT firstname, incamount from employee5 A
INNER JOIN incentive B on A.empid = B.empid 
AND incamount > 3000
) select * from join2_cte


with join3_cte(first_name,incamount) as (
SELECT firstname, incamount from employee5 A
LEft  JOIN incentive B  on
A.empid = B.empid
) select * from join3_cte


with join4_cte(empname,manager) as (
SELECT e.firstname as empname , 
m.firstname as manager
from employee5 e LEFT OUTER JOIN employee5 m on m.empid = e.managerid
) select * from  join4_cte


with join5_cte(first_name,nullToZero) as (
SELECT firstname,  isnull(incamount,0) from employee5 A
LEft  JOIN incentive B  on
A.empid = B.empid) select * from join5_cte
